...
export default function AllStories() {
  ...
  return (
    <div className='flex flex-col items-center w-full px-[16px]'>
        ...
        {stories.map((data) => {
          if(currDate !== new Date(data?.publish_date)?.toJSON()?.slice(0, 10).replace(/-/g, '/')){
            return null
          } else {
            return(
        ...
        <div className='mt-[20px] md:mt-[80px] sm:px-[30px] w-full flex flex-col items-center'>
          ...
          >
              {activeIndexYes + 1 < stories.length - 1 || stories.length > 3 && (
                  <div
                  onClick={() => {
              ...
              >
                  {stories.map((item, index) => {
                    if(hello(item.publish_date) !== yesterday.toDateString()){
                      return null
                    } else {
                      return <StoryItem story={item} key={index} index={index} />;
                    }
