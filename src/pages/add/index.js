...
import { CATEGORIES, POST, CREATE, UPDATE } from './_gql';
import { sortBy } from 'lodash';
import { Button, Card, Checkbox, Col, Form, Image, Input, Row, Select, Skeleton, Upload } from 'antd';
import {
  CameraFilled,
  DeleteOutlined,
  FileImageOutlined,
...
function AddPost() {
  ...
  const { saveArticle, loading: saving } = useMutation(id ? UPDATE : CREATE);
  useEffect(() => {
      ...
      <Row gutter={12}>
        <Col span={16} className="w-full mx-[50px]">
          <Card bordered={false} className="max-w-[700px]">
            ...
            <Button title="Save" size="large" icon={<SaveOutlined />} htmlType="submit" loading={saving}>
              Save
            </Button>
          </Card>
        </Col>
        <Col span={6}>
          ...
          <Form.Item name="tagIds" className="font-merri">
...
function ImageBlock({ block, idx }) {
  return (
    ...
    >
      <Row gutter={12}>
        <Col span={8}>
          <Image src={imagePath(block.imageUrl) || fallback} fallback={fallback} />
          <Upload name="image" multiple={false}>
...
export function TextBlock({ block, idx }) {
  ...
  return (
    ...
    >
      ...
      <Form.Item name={['blocks', idx, 'data', 'text_type']}>
