...
import { FontSizeOutlined, PaperClipOutlined, YoutubeFilled } from '@ant-design/icons';
...
export default function SortableContainer({ items, setItems }) {
          ...
          >
            <SortableContext items={items} strategy={horizontalListSortingStrategy}>
              {fields.map((field, idx) => (
                <SortableBlock block={items[idx]} key={idx} />
              ))}
            </SortableContext>
            <DragOverlay>{activeId ? <Skeleton.Image /> : null}</DragOverlay>
          </DndContext>
        </>
      )}
    </Form.List>
  ...
  function handleDragEnd(event, move) {
    const { active, over } = event;

    if (active.id !== over.id) {
      setItems((items) => {
        const oldIndex = items.findIndex((x) => x.position === active.id);
        const newIndex = items.findIndex((x) => x.position === over.id);
...
function SortableBlock({ block }) {
  const { attributes, listeners, isDragging, setNodeRef, transform, transition } = useSortable({ id: block?.position });
  const style = {
