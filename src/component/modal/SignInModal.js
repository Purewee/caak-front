...
function LoginWithMail({ onSuccess, setIsShown }) {
  ...
  return (
    <div className='popup_modal-content w-[380px] rounded-[6px] relative'>
      ...
      >
        <p className='mt-[60px] text-[32px] text-[#111111] leading-[38px] font-condensed font-bold'>Нэвтрэх</p>
        <input {...register('username') }  className='w-full rounded-[4px] mt-[30px] bg-[#EFEEEF] h-[44px] px-[22px]'  placeholder='Имэйл хаяг/Утасны дугаар' name='username'/>
        <div className='w-full relative mt-[12px]'>
          <input {...register('password') } type={password ? 'text' : 'password'} className='w-full rounded-[4px] bg-[#EFEEEF] h-[44px] pl-[22px] pr-[40px]'  placeholder='Нууц үг'/>
          <span onClick={() => setPassword(!password)} className={`${password ? 'icon-fi-rs-blind' : 'icon-fi-rs-eye'} absolute right-[12px] text-[#BBBEBE] cursor-pointer text-[20px] w-[24px] h-[24px] top-[10px] flex items-center justify-center leading-[24px]`} />
        </div>
