      ...
      >
        <div className=" flex flex-col">
          <Link to={`/post/view/${post.id}`}>
            <img alt="" src={`http://graph.caak.mn${post.image}`} className="min-h-[220px] max-h-[220px] sm:min-h-[300px] sm:max-h-[300px] w-full object-cover" />
          </Link>
        ...
        <div className="flex flex-row xl:flex-col">
          ...
          <div className={`relative flex flex-col pt-[6px] justify-between md:pb-[21px] md:h-[223px]`}>
            <div className="flex flex-col w-full">
              {isLaptop && post.categories.map((x) => (
              ...
              {isLaptop && <p className="text-[#909090] text-[14px] mt-[10px]">{new Date(post.publish_date).getFullYear() + '.' + (new Date(post.publish_date).getMonth() + 1) + '.' + new Date(post.publish_date).getDate() + ', ' + new Date(post.publish_date).getHours() + ':' + new Date(post.publish_date).getMinutes()}</p>}
            </div>
